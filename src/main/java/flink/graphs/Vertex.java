...
public class Vertex<K extends Comparable<K> & Serializable, V extends Serializable> extends Tuple2<K, V> {
     ...
     */
    public <EV extends Serializable> DataSet<Integer> inDegree(ExecutionEnvironment environment,
                                                               Graph<K, V, EV> graph) {
     ...
     */
    public <EV extends Serializable> DataSet<Integer> outDegree(ExecutionEnvironment environment,
                                                                Graph<K, V, EV> graph) {
        return VertexUtils.countWhere(environment, graph, new FilterFunction<Tuple3<K, K, EV>>() {
            @Override
            public boolean filter(Tuple3<K, K, EV> edge) throws Exception {
     ...
     */
    public <EV extends Serializable> DataSet<Integer> getDegree(ExecutionEnvironment environment,
                                                                Graph<K, V, EV> graph) {
        return VertexUtils.countWhere(environment, graph, new FilterFunction<Tuple3<K, K, EV>>() {
            @Override
            public boolean filter(Tuple3<K, K, EV> edge) throws Exception {
